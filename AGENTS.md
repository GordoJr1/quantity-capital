...
python3 scripts/ingest_canada_mine_production.py --sqlite qc.sqlite --apply  # monthly; cited 2025 + labeled 2026 YTD mine production → qc.sqlite → thin export
```
...
- Pages: `index.html`, `politician.html`, `ticker.html`, `signals.html`, `landed.html`, `tells.html`, `paper.html`, `canada.html`, and the `insider*.html` set. Beta chrome: `beta.html` (Mines), `claims.html` (Claims), `canada.html` (Canada).
- Scripts: `qc.js` (shared app logic), `chart.js`, `refresh.js`, `sw.js`.
- Data: `trades*.json`, `bios.json`, `tickers.json`, `traders.json`, `analysis.json`, `tells.json`, `backtest.json`, `insider-*.json`, `canada/commodities.json`, `canada/producer-join.json`, `beta/<issuer>.json`, and per-ticker `prices/<TICKER>.json`.
