...
`.cursor/environment.json` runs this in `install`, so a fresh Cloud Agent always has an up-to-date `backtest.json`.
...
`README.md` references a collection pipeline (`collect/insider_collect.py`, `collect/fetch_insider_prices.py`, `collect/build_insider_analysis.py`) that refreshes the insider datasets. Those scripts are **not present** in this repository, so they are intentionally left out of the environment setup. If they are added, they will likely need outbound network access (SEC/SEDI/CEO.CA) and possibly credentials; wire them into `.cursor/environment.json` and the egress allowlist at that point.
