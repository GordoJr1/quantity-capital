...
def pick_follow(repeatable: dict) -> list[dict]:
    rows = []
    ...
    rows.sort(key=lambda r: (-(r.get("avg") or -99), -(r.get("hit") or 0), -(r.get("n") or 0), r.get("name") or ""))
    for i, row in enumerate(rows, 1):
        row["rank"] = i
    return rows


def cluster_prints(trades: list[dict], follow_ids: set[str]) -> dict[str, dict]:
...
def choose_alerts(
    ...
    if not prev:
        ...
        alerts.sort(key=lambda r: (r["filed_date"], r["filer"], r["ticker"]), reverse=True)
        return alerts
...
def build(args: argparse.Namespace) -> dict:
    ...
    follow = pick_follow(repeatable)
    follow_ids = {r["id"] for r in follow if r["id"]}
    clusters = cluster_prints(tape.get("trades") or [], follow_ids)
