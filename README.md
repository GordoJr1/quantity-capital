...
python3 scripts/ingest_canada_mine_production.py --apply   # cited 2025 mine ounces → sidecar + overlay
python3 scripts/ingest_canada_mine_production.py --check
```

Writes `canada/commodities.json` and `canada/mine-production.json`. No secrets. Do not commit `qc.sqlite`. Mine-level tonnes from StatCan/NRCan are not published and are never invented. Optional 2025 mine production is company-disclosed only (IR / EDGAR / MD&A), troy ounces for precious metals. See `canada/README.md`.
